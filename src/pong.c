...
int main() {
    int r1x = 3, r1y1 = 12, r1y2 = 13, r1y3 = 14, x_len = 82, y_len = 27, move_count = 3, pointsr = 0;
    ...
    while (pointsl < 21 && pointsr < 21) {
        ...
        if (move_count == 3) {
            flag = flag_check(flag, ball_y);
            flag = ball_bounce_l(flag, ball_x, ball_y, r1y1, r1y2, r1y3);
            flag = ball_bounce_r(flag, ball_x, ball_y, r2y1, r2y2, r2y3);
            ball_x = ball_move_x(ball_x, flag), ball_y = ball_move_y(ball_y, flag);
            ...
            if (point_check_(ball_x) == 1) {
                pointsr = pointerr(ball_x, pointsr);
                pointsl = pointerl(ball_x, pointsl);
                ball_x = 40;
            }
            move_count = 0;
        }
        move_count++, scanf("%c", &move);
    }
    winner(pointsr, pointsl);
    return 0;
}
